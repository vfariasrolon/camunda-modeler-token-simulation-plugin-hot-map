...
export default class Heatmap {
  ...
  _init() {
    ...
    // Add controls
    const radiusPlusButton = domify(`<div class="bts-entry" title="Increase Radius">${PlusIcon('radius-plus')}</div>`);
    domEvent.bind(radiusPlusButton, 'click', () => this._adjustRadius(5));
    this._tokenSimulationPalette.addEntry(radiusPlusButton, 7);

    const radiusMinusButton = domify(`<div class="bts-entry" title="Decrease Radius">${MinusIcon('radius-minus')}</div>`);
    domEvent.bind(radiusMinusButton, 'click', () => this._adjustRadius(-5));
    this._tokenSimulationPalette.addEntry(radiusMinusButton, 8);

    const blurPlusButton = domify(`<div class="bts-entry" title="Increase Blur">${PlusIcon('blur-plus')}</div>`);
    domEvent.bind(blurPlusButton, 'click', () => this._adjustBlur(5));
    this._tokenSimulationPalette.addEntry(blurPlusButton, 9);

    const blurMinusButton = domify(`<div class="bts-entry" title="Decrease Blur">${MinusIcon('blur-minus')}</div>`);
